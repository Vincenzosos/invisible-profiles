# ==============================================================================
# STEP 6: PCA + FACTOR ANALYSIS — SWEDEN
# Thesis: "Beyond the Monolith"
# SHARE Wave 9, Swedish 65+
#
# Questo script è analogo a Step 5 Italia, con adattamenti per la Svezia:
#   Part A — Dimension-wise PCA (5 dimensioni separate)
#   Part B — Factor Analysis su tutte le variabili (6 fattori, varimax)
#
# DIFFERENZE CHIAVE vs ITALIA:
#   - Input: step4_sweden_std.rds (Svezia passa per step3→step4, non step2)
#   - ac035d4/ac035d7 in Svezia hanno varianza bassa ma non zero (brief: 4-6%)
#     quindi saranno tutte 31 variabili attive (vs 29 in Italia).
#   - Atteso Heywood case su social_integration (loading 1.09 > 1, brief p.181).
#     Lo script avvisa ma non si blocca.
#   - Digital: brief prevede 4 PC (55.0% var). Se Kaiser dà 3, forziamo 4
#     per coerenza col disegno originale (come in Italia).
#
# INPUT:  v9/step4_sweden_std.rds    (standardizzate, per PCA/FA)
#         v9/step4_sweden_clean.rds  (valori originali, per descrittiva)
# OUTPUT: v9/step6_sweden_pca_results.rds
#         v9/step6_sweden_fa_results.rds
#         v9/step6_sweden_with_fa_scores.rds
# ==============================================================================
...
cat("============================================================\n")
cat("STEP 6: PCA + FACTOR ANALYSIS — SWEDEN\n")
cat("============================================================\n\n")
...
sweden_clean <- readRDS(file.path(data_path, "v9", "outputs", "step4_sweden_clean.rds"))
cat("  Loaded step4_sweden_std.rds:",   nrow(sweden_std),   "rows\n")
cat("  Loaded step4_sweden_clean.rds:", nrow(sweden_clean), "rows\n\n")

# --- Definizione variabili (identica a Italia per simmetria cross-country) ---
...
# --- Check varianza zero (in Svezia attendiamo nessuna zero-variance) ---
...
# --- A1. HEALTH (8 variabili → attesi 2 PC, ~56% var expl) ---
pca_health <- run_pca_block(sweden_std, health_vars, "Health")

# --- A2. ECONOMIC (5 variabili → attesi 2 PC, ~71% var expl) ---
pca_econ <- run_pca_block(sweden_std, econ_vars, "Economic")

# --- A3. DIGITAL/SOCIAL (fino a 10 variabili → attesi 4 PC, ~55% var expl) ---
# In Svezia tutte e 10 le variabili digitali dovrebbero avere varianza (brief: 4-6%
# per ac035d4/ac035d7). Se Kaiser non dà 4 PC, forziamo 4 per coerenza col brief.
# NB: `digital_active` = digital_vars senza zero-variance (in Svezia = tutte 10).
pca_digital <- run_pca_block(sweden_std, digital_active, "Digital/Social", force_n = 4)

# --- A4. COGNITIVE (3 variabili → atteso 1 PC, ~48% var expl) ---
pca_cog <- run_pca_block(sweden_std, cog_vars, "Cognitive")

# --- A5. SUBJECTIVE (5 variabili → atteso 1 PC, ~33% var expl) ---
pca_subj <- run_pca_block(sweden_std, subj_vars, "Subjective")
...
X_fa <- as.matrix(sweden_std[, active_vars])
R_fa <- cor(X_fa)
...
# --- B3. SCREE (eigenvalues full) --------------------------------------------

eig_full <- eigen(R_fa)$values
cat(sprintf("--- Eigenvalues (full correlation matrix, %d variabili) ---\n",
            length(active_vars)))
...
# ATTENZIONE: brief (p.181) segnala un Heywood case su social_integration in
...
fa_result <- withCallingHandlers(
  ...
  warning = function(w) {
    if (grepl("Heywood|ultra-Heywood", w$message, ignore.case = TRUE)) {
      cat("  ⚠ Heywood case rilevato (atteso dal brief p.181):\n")
      cat("    ", w$message, "\n")
...
# Loadings
cat("\n  FACTOR LOADINGS (varimax, cut = 0.30):\n")
print(fa_result$loadings, cutoff = 0.30, sort = TRUE)

cat("\n  FACTOR LOADINGS (completi, ordinati per fattore dominante):\n")
...
# Flag esplicito di Heywood cases nei loadings
heywood_vars <- names(fa_result$communality[fa_result$communality > 1])
if (length(heywood_vars) > 0) {
  cat("\n  ⚠ HEYWOOD CASES (communalità > 1):\n")
  for (hv in heywood_vars) {
    cat(sprintf("    %s: h2 = %.3f\n", hv, fa_result$communality[hv]))
  }
  cat("    (Indica possibile over-parameterizzazione o correlazione estrema.\n")
  cat("     Atteso in Svezia su social_integration — vedi brief p.181.)\n")
}
...
for (v in active_vars[order(-comm)]) {
  h2 <- comm[v]
  quality <- if (h2 > 1)       "HEYWOOD (>1)"
             else if (h2 > 0.5) "buona"
             else if (h2 > 0.3) "moderata"
             else               "bassa"
...
cat("  dominante e matchato con le label del brief per overlap di variabili chiave.\n")
...
cat("  Il brief (linee 174-179) assegna label ai fattori in modo indipendente per\n")
cat("  IT e SE, quindi i match qui sono attesi ma i numeri possono differire.\n\n")
...
pca_results <- list(
  ...
  n_obs             = nrow(sweden_std)
)
saveRDS(pca_results, file.path(data_path, "v9", "outputs", "step6_sweden_pca_results.rds"))
cat("  Salvato: v9/step6_sweden_pca_results.rds\n")
...
fa_results <- list(
  ...
  fa_course       = fa_course,
  heywood_vars    = heywood_vars,       # variabili con h2 > 1 (se presenti)
  fit_stats       = list(
    RMSR  = fa_result$rms,
    TLI   = fa_result$TLI,
...
saveRDS(fa_results, file.path(data_path, "v9", "outputs", "step6_sweden_fa_results.rds"))
cat("  Salvato: v9/step6_sweden_fa_results.rds\n")

sweden_with_scores <- cbind(sweden_clean, factor_scores)
saveRDS(sweden_with_scores, file.path(data_path, "v9", "outputs", "step6_sweden_with_fa_scores.rds"))
cat("  Salvato: v9/step6_sweden_with_fa_scores.rds (dati + factor scores)\n")
...
cat("\n============================================================\n")
cat("RIEPILOGO STEP 6 — SWEDEN\n")
cat("============================================================\n")
cat("  Osservazioni:       ", nrow(sweden_std), "\n")
cat("  Variabili totali:    31\n")
cat("  Variabili escluse:  ",
    ifelse(length(zero_vars) > 0, paste(zero_vars, collapse = ", "), "nessuna"),
    ifelse(length(zero_vars) > 0, "(varianza zero)", ""), "\n")
...
if (length(heywood_vars) > 0) {
  cat(sprintf("    Heywood cases: %s\n",
              paste(heywood_vars, collapse = ", ")))
}
cat("\n")

cat("  NOTA IMPORTANTE:\n")
cat("  Il clustering (Step 7+) si farà sulle 31 variabili ORIGINALI STANDARDIZZATE\n")
cat("  (step4_sweden_std.rds), NON sui factor scores.\n")
cat("  I factor scores servono SOLO per le mappe fattoriali post-clustering.\n")

cat("\n============================================================\n")
cat("STEP 6 COMPLETATO.\n")
cat("Prossimo step: Clustering Italy (Step 7) o Clustering Sweden (Step 8)\n")
cat("============================================================\n")
