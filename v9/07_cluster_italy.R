...
#
# INPUT:  v9/step2_italy_std.rds            (standardizzate, per clustering)
#         v9/step2_italy_clean.rds          (valori originali, per descrittive)
#         share_italy_all_approaches.rds    (demographics esterni via mergeid)
# OUTPUT: v9/step7_italy_kmeans_k6.rds      (pre-micro-cluster removal)
#         v9/step7_italy_kmeans_k5.rds      (finale, 5 profili)
#         v9/step7_italy_with_clusters.rds  (dati + cluster + labels)
#         v9/step7_italy_profile_table.rds  (tabella riassuntiva profili)
# ==============================================================================
...
cluster_stats <- italy_clean_core %>%
  ...
  summarise(
    ...
    lone_mean    = mean(loneliness, na.rm = TRUE),
    internet_pct = mean(internet, na.rm = TRUE),   # binary, mean = proportion
...
saveRDS(kmeans_k6_results, file.path(data_path, "v9", "outputs", "step7_italy_kmeans_k6.rds"))
cat("  Salvato: v9/step7_italy_kmeans_k6.rds\n")
...
saveRDS(kmeans_k5_results, file.path(data_path, "v9", "outputs", "step7_italy_kmeans_k5.rds"))
cat("  Salvato: v9/step7_italy_kmeans_k5.rds\n")

# Step 7c: dati arricchiti con cluster e profile label
italy_with_clusters <- italy_clean_core
saveRDS(italy_with_clusters, file.path(data_path, "v9", "outputs", "step7_italy_with_clusters.rds"))
cat("  Salvato: v9/step7_italy_with_clusters.rds\n")

# Step 7d: tabella riassuntiva profilo (solo il pezzo pubblicabile)
saveRDS(profile_table, file.path(data_path, "v9", "outputs", "step7_italy_profile_table.rds"))
cat("  Salvato: v9/step7_italy_profile_table.rds\n")
