...
\begin{table}[H]
\centering
\caption[Sensitivity analyses: summary against interpretive thresholds]{Sensitivity analyses: summary against interpretive thresholds.}
...
\par\medskip
\begin{minipage}{\linewidth}
\footnotesize\textit{Note.} Unweighted shares are within-sample cluster fractions. Weighted shares are computed as $\sum_{i \in C} w_i / \sum_i w_i$ using the SHARE Wave~9 calibrated cross-sectional individual weight (\texttt{cciw\_w9}). The population projection in Figure~\ref{fig:ch4-shares} applies the weighted shares to the Istat 2024 Italian over-65 population estimate (14.18~million). Differences computed on unrounded shares.
\end{minipage}
\end{table}


\chapter{SHARE Data and Funding Acknowledgement}
\label{ch:appx-share-funding}

The empirical analysis uses data from SHARE Wave~9 (DOI: 10.6103/SHARE.w9.900) \citep{share_w9_2024} and, for the cross-wave analysis of \S\ref{sec:ch7-cross-wave}, SHARE Wave~8 (DOI: 10.6103/SHARE.w8.900) \citep{share_w8_2024}, release version 9.0.0; see \citet{borschsupan2013} for methodological details. The funding acknowledgement below is reproduced in full as required by the SHARE-ERIC Conditions of Use.
